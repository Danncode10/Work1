...
- ✅ Added AWS EC2 security group rule to allow inbound traffic on port 5173 for frontend development server
- ✅ Updated frontend startup command to use `--host 0.0.0.0` flag for network accessibility
...
```bash
cd ui
npm run dev -- --host 0.0.0.0  # Starts Vite dev server accessible on network
```
...
1. **Local Development - Frontend:**
   ...
   ```
   Access app at: http://localhost:5173 or public IP
